...
symptoms_sheffield_etl <- function(provenance,log_dir=NULL) {
  if (str_contains(input_dir,"Sheffield")) {
    # sheffield has no symptomatalogy, so the dataframe should just have the standard column names but no data rows.
  ...
  if (!is.null(log_dir)) {
    write.csv(symptoms_df,path(log_dir,"0 symptoms_df_after_etl_sheffield.csv"),row.names=FALSE)
  }
