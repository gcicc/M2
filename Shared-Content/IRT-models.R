...
#' 
#' 
common.alpha.model <- function(data.in = for.IRT.modeling){
  lin <- "lin"  # We're using linear
  respf<- "NV"  # We're using normal
  indicator<- "C" # for continuous
  ...
  # Linear item difficulty function
  diffunct<-diffunctlin

  # Derivative of linear item difficulty function
  derdiffunct<-derdiffunctlin

  for.model <- t(data.in %>% dplyr::select(-SUBJID))
  Thrp<-ThreshModFixedResp(dat=for.model,
                           commonslope="var" ,
                           indicator=indicator,
                           der="der",
                           lin =lin,
...
#'
#' @examples
uncommon.alpha.model <- function(data.in = for.IRT.modeling){
  lin <- "lin"  # We're using linear
  respf<- "NV"  # We're using normal
  indicator<- "C" # for continuous
  ...
  derdiffunct<-derdiffunctlin
  for.model <- t(data.in %>% dplyr::select(-SUBJID))
  Thralpha<- ThreshModFixedAlphaResp(for.model,commonslope="var" ,indicator=indicator, der="der",lin =lin, start, startind="no",
                                     penalpha=0,respf)
  return(Thralpha)
}
...
derdiffunct <- function(intdiff,slopediff,y){
  r<- slopediff
  return(r)
}
respfctder <- function(x){
  r<--x*dnorm(x)
  return(r)
}
